...
pub fn serialize_ciphertext_json(ciphertext: &CipherText) -> String {
    to_string(ciphertext).expect("JSON Serialization failed")
}

// Deserialize CipherText from JSON
pub fn deserialize_ciphertext_json(data: &str) -> CipherText {
    from_str(data).expect("JSON Deserialization failed")
}

// Test the correctness of the serialization/deserialization process
pub fn test_serialization(ciphertext: &CipherText) {
    let serialized = serialize_ciphertext_json(ciphertext);
    let deserialized: CipherText = deserialize_ciphertext_json(&serialized);
