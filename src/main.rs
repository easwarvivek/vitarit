...
pub fn keygen() -> KeyPair {
    ...
    let hash = Sha512::digest(dk.as_bytes());
    let mut hash_bytes = [0u8; 64];
    hash_bytes.copy_from_slice(&hash);
    let ek = RistrettoPoint::from_uniform_bytes(&hash_bytes);
...
pub fn encrypt(ek: &RistrettoPoint, message: &[u8]) -> CipherText {
    ...
    let c1 = RistrettoPoint::multiscalar_mul(&[r], &[RistrettoPoint::default()]).compress();
    let c2 = RistrettoPoint::multiscalar_mul(&[r, s], &[ek.clone(), RistrettoPoint::default()]).compress();
    
    let mut hasher = Sha512::new();
    hasher.update(RistrettoPoint::multiscalar_mul(&[s], &[RistrettoPoint::default()]).compress().as_bytes());
    ...
    CipherText {
        ...
        c2: c2.into(),
        c3,
        s: s.into(),  // Store `s` as a serializable scalar
    }
}

// Decryption
pub fn decrypt(dk: &Scalar, ciphertext: &CipherText) -> Vec<u8> {
    let r_point: CompressedRistretto = ciphertext.c1.clone().into();
    let s: Scalar = ciphertext.s.clone().into();  // Retrieve `s` as a scalar

    let r_point = r_point.decompress().expect("Decompression failed");
    let s_point = RistrettoPoint::multiscalar_mul(&[s], &[RistrettoPoint::default()]);
...
fn main() {
    ...
    println!("Original message: {:?}", String::from_utf8(decrypted_message).unwrap());
}
*/
