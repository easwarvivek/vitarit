...
use crate::scalar_bls12381::{curve_order, field_add_assign, field_mul, field_mul_assign, rand_scalar, scalar_one};
...
pub fn verify_sharing(
    ...
    // == g_2^z_alpha
    let mut kbig = BIG::new(); // David: for k
    let one = scalar_one();
    let mut lhs = ECP::new();
    instance.public_coefficients.iter().for_each(|aa_k| {
        let mut acc = BIG::new();
        let mut xpow = x; // David: for x^i
        let mut ibig = scalar_one(); // David: for i (in i^k)
        instance.public_keys.iter().for_each(|_| {
            let tmp = field_mul(&ibig.powmod(&kbig, &curve_order()), &xpow);
            field_add_assign(&mut acc, &tmp);
            field_mul_assign(&mut xpow, &x);
            field_add_assign(&mut ibig, &one);
        });
        lhs.add(&pair::g1mul(&aa_k, &acc));
        field_add_assign(&mut kbig, &one);
    });
