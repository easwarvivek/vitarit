...
mod test {
    ...
    use crate::bls_signature::{public_key_from_secret_key, sign_message, verify_message_signature};
    ...
    fn test_threshold_signatures(
        ...
        let signatures: Vec<_> = secret_keys.iter()
            .map(|secret_key| sign_message(message, secret_key))
            .collect();

        let pubkeys: Vec<_> = secret_keys.iter()
            .map(|secret_key| public_key_from_secret_key(secret_key))
            .collect();
        ...
        for (index, (signature, pubkey)) in signatures.iter().zip(pubkeys).enumerate() {
            ...
            assert!(verify_message_signature(message, signature, &pubkey));
            ...
            if threshold > 1 {
                ...
                assert!(!verify_message_signature(message, signature, &wrong_public_key),
        ...
        let public_key = combine_public_keys(&pubkey_options[..], threshold).unwrap();
        let secret_key = combined_secret_key(secret_keys);
        assert!(public_key.equals(&public_key_from_secret_key(&secret_key)));

        // Combine a random subset of signatures:
        let signature_selection = select_n(seed, threshold, &signatures);
        let signature = combine_signatures(&signature_selection, threshold)
            .expect("Failed to combine signatures");

        // Correct values validate:
        assert!(verify_message_signature(message, &signature, &public_key));
        ...
        if !public_coefficients.coefficients.is_empty() {
            ...
            assert!(!verify_message_signature(&incorrect_message, &signature, &public_key));
        }
        if public_coefficients.coefficients.len() > 1 {
            let some_individual_signature = signatures[0].clone();
            assert!(
                !verify_message_signature(message, &some_individual_signature, &public_key),
    ...
    fn omnipotent_dealer() {
        ...
        let combined_signature = combine_signatures(&signature_options, threshold)
        ...
        let public_key = combine_public_keys(&public_keys, threshold).unwrap();
        assert!(verify_message_signature(&message[..], &combined_signature, &public_key));
    }
