...
pub fn hash_to_ecp(dst: &[u8], msg: &[u8]) -> ECP {
...
///
/// # Arguments
/// * `dst` a domain separator (see the internet draft for guidance on
///   formatting)
/// * `mess` is a message that will be hashed onto G2
/// # Returns
/// An element of BLS12-381 G2
pub fn hash_to_ecp2(dst: &[u8], msg: &[u8]) -> ECP2 {
...
mod test {
    ...
    fn test_distinct_messages_yield_distinct_curve_points_ecp() {
        ...
        let points: HashSet<_> = (0..number_of_messages as u32)
            .map(|number| {
                let miracl_g1 = hash_to_ecp(dst, &number.to_be_bytes()[..]);
    ...
    fn test_distinct_messages_yield_distinct_curve_points_ecp2() {
        ...
        let points: HashSet<_> = (0..number_of_messages as u32)
            .map(|number| {
                let miracl_g2 = hash_to_ecp2(dst, &number.to_be_bytes()[..]);
